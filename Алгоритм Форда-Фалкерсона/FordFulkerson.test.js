...
describe('FordFulkerson Algorithm Tests', () => {
    ...
    function measureExecutionTime(graph) {
        const ff = new FordFulkerson(graph);
        const source = 0;
        const sink = graph.V - 1;

        const startTime = performance.now();
        ff.maxFlow(source, sink);
    ...
    test('Тест 6: Проверка временной сложности', () => {
        ...
        for (let i = 1; i < timings.length; i++) {
            ...
            const theoreticalRatio = (current.V * Math.pow(current.E, 2)) / (previous.V * Math.pow(previous.E, 2));
